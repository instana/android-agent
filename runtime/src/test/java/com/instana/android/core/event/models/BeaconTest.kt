...
class BeaconTest: BaseTest() {
    ...
    @Test
    fun `test beacons class with setMobileFeatures list is been added with perf, drop & W3CHeader`(){
        val config = InstanaConfig(InstanaTest.API_KEY, InstanaTest.SERVER_URL, enableCrashReporting = true)
        Instana.sessionId = null
        SessionService(app,mockWorkManager,config)
        val customEvent = Beacon.newCustomEvent(
        ...
        customEvent.setMobileFeatures(listOf(MobileFeature.CRASH,MobileFeature.LOW_MEMORY,MobileFeature.DROP_BEACON,MobileFeature.ANR, MobileFeature.W3C_HEADER))
        assert(customEvent.toString().contains("uf\tc,lm,db,anr,ot"))
    }
