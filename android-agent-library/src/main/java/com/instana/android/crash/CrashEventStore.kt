...
object CrashEventStore {
    ...
    private const val KEY_WORKER_JSON = "worker_json"
    ...
    val json: String
        get() {
            return pref?.getString(KEY_WORKER_JSON, EMPTY_STR) ?: EMPTY_STR
        }
    ...
    fun saveEvent(tag: String, json: String) {
        editor = pref?.edit()
        editor?.apply {
            putString(KEY_WORKER_TAG, tag)
            putString(KEY_WORKER_JSON, json)
        }?.commit()
    }
